...
- 장점 :
    1) props를 활용한 조건부 스타일링이 가능하다.
    2) 한 컴포넌트 내에서 css 코드를 함께 작업할 수 있다.
    3) 리액트 식의 컴포넌트 분리가 편해서 재사용성이 높다.
- 단점 :
    1) css를 줄 태그를 일일이 컴포넌트로 만들어야 한다.
    2) CSS 수정할 때마다 해당 컴포넌트 파일 위치를 찾아야 하는 번거로움이 있다.
